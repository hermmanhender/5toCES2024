...
env_config={
    ...
    "infos_variables": ["ppd", 'heating', 'cooling', 'occupancy'],
    ...
    'beta_reward': 0.5, # más cerca de 0, mayor confort.
...
if algorithm == 'PPO': # PPO Configuration
    algo = PPOConfig().training(
        ...
        exploration_config = {
            ...
            "initial_epsilon": 1.,
            "final_epsilon": 0.,
            "epsilon_timesteps": 6*24*365*8*5,
        },
    ).multi_agent(
        policies = {
...
def trial_str_creator(trial):
    ...
    """
    return "OnOffHVAC_beta05_{}_{}".format(trial.trainable_name, trial.trial_id)
...
if not restore:
    tune.Tuner(
        ...
        tune_config=tune.TuneConfig(
            ...
            #     max_t= 6*24*365*2*3,
            #     grace_period=6*24*365,
            # ),
            
            
        ),
        run_config=air.RunConfig(
            name = "{date}_OnOffHVAC_marl_{algorithm}".format(
                date = time.strftime("%Y%m%d%H%M%S"),
                algorithm = str(algorithm),
            ),
            storage_path = 'C:/Users/grhen/ray_results',
            stop = {"episodes_total": 8*10},
            log_to_file = True,
